...
public class HtmlParser {
    ...
    public static List<PantoneColor> getPontoneColor(String file) {
        List<PantoneColor> pantoneColors = new ArrayList<PantoneColor>();
        Document doc = Jsoup.parse(readHtml("src/main/resources/file/111.html"));
        // 获取html的标题
        String title = doc.select("title").text();
        System.out.println(title);
        // 获取a标签色卡数据
        Elements elements = doc.select(".category_colors").select("a");
        System.out.println(elements.size());
        for (Element e : elements) {
            String href = e.attr("href");
            String id = href.substring(href.indexOf("#/") + 2, href.lastIndexOf("/"));
            String a_title = e.attr("title");
            String number = a_title.substring(a_title.indexOf("PANTONE ") + 8, a_title.lastIndexOf("TPX"));
            number = number.replaceAll("\\s*", "");
            a_title = number + "TPG";
            a_title = "PANTONE " + a_title;
            String style = e.attr("style");
            System.out.println(id + "  " + number + "  " + a_title + "  " + style);
            pantoneColors.add(new PantoneColor(id, number, a_title, style));
        }
...
//        Elements elements = doc.select(".head_wrapper").select("#u1").select("a");
        ...
        for (Element e : elements) {
            ...
            a_title = number + "TPG";
            a_title = "PANTONE " + a_title;
            System.out.println(href.substring(href.indexOf("#/") + 2, href.lastIndexOf("/")) + "  " + number + "  " + a_title + "  " + e.attr("style"));
        }
